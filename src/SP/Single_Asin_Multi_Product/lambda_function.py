...
# ------------------ KEYWORD ------------------
# def create_keyword(access_token, campaign_id, ad_group_id, keyword_text, match_type, bid, state):
...
def create_product_target(access_token, creds, cid, agid, asin, bid, state):
    ...
    r = requests.post(url, headers=headers, json=payload)
    print("Target Response:", r.text)
    data = r.json()
    if "success" in data.get("targetingClauses", {}):
        return data["targetingClauses"]["success"][0]["targetId"]
    else:
        print("Product target creation failed:", data)
        return None
...
def lambda_handler(event, context):
    ...
    target_id = create_product_target(access_token, creds, cid, agid, asin, bid, state)
    if target_id:
         product_targets.append(target_id)

    # kid = create_keyword(access_token, cid, agid, keyword_text, match_type, bid, state)
    # pid = create_product_ad(cid, agid, asin, sku, access_token, state)
    # target_id = create_product_target(access_token, cid, agid, asin, bid, state)

    return {
        "campaignId": cid,
        "adGroupId": agid,
        # "keywordId": kid,
        "productAdId": product_ads,
