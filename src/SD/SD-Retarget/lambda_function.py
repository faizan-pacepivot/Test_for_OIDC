...
# ------------------ CAMPAIGN ------------------
def build_Campaign_Payload_SD_Retarget(name, budgetType, budget, startDate, state, tactic):
    return [{
                "name": name,
...
def create_campaign(access_token, creds, name, budgetType, budget, startDate, state, tactic):
    ...
    payload = build_Campaign_Payload_SD_Retarget(name, budgetType, budget, startDate, state, tactic)
...
# ---------------------Retarget Targeting -----------------------------
def create_toys_retarget_targeting(access_token, creds, agid, bid, state, asin):
    ...
    print("🎯 RETARGET Payload:", json.dumps(payload, indent=2))
    r = requests.post(url, headers=headers, json=payload)
    print("✅ RETARGET Response:", r.status_code, r.text)
    return r.json()
...
def lambda_handler(event, context):
    ...
    retarget_result = create_toys_retarget_targeting(access_token, creds, agid, bid_retarget, state, asin)
    # else:
    ...
    return {
        ...
        "toysRetargetResult": retarget_result
    }
